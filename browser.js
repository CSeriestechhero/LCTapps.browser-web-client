  ...
  Browser.prototype.init = function() {
    (function(browser) {
	  ...
	  browser.locationForm.addEventListener('submit', function(e) {
        e.preventDefault();
		var str = browser.locationBar.value;
		if(str.substring(0, 3) !== "http")
			{
				if(str.substring(str.length - 4, str.length) !== ".com")
				{
					browser.tabs.getSelected().navigateTo("https://www.google.com/search?q=" + browser.locationBar.value);
				}
			}
		if(str.substring(str.length - 4, str.length) === ".com")
			{
				browser.tabs.getSelected().navigateTo("http://" + str);
			}
